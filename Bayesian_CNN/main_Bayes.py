...
def train(epoch):
    ...
    for batch_idx, (inputs_value, targets) in enumerate(trainloader):
        ...
        sys.stdout.write('| Epoch [%3d/%3d] Iter[%3d/%3d]\t\tLoss: %.4f Acc@1: %.3f%%' %(epoch, num_epochs, batch_idx+1,
                    (len(trainset)//batch_size)+1, loss.data.item(), (100*correct/total)/args.num_samples))
        sys.stdout.flush()

    diagnostics_to_write = {'Epoch': epoch, 'Loss': loss.data.item(), 'Accuracy': (100*correct/total)/args.num_samples}
...
def test(epoch):
    ...
    test_diagnostics_to_write = {'Validation Epoch': epoch, 'Loss': loss.data.item(), 'Accuracy': acc}
    with open(logfile, 'a') as lf:
